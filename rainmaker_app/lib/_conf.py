    ...
    def subst_all(self):
        ...
        for k in self:
            if k in self.d:
                self.d[k].subst_all()
                next
            ktype = self.meta(k)['type']

            #All types starting with arr are arrays
            if ktype.startswith('arr'):
                ...
                vals=[]
                for val in self[k]:
                    vals.append(self.subst(val))
            else:
                self[k]=self.subst(self[k])
            
            # compile regexes 
            if ktype =='array_fnmatch':
                vals=[]
                for reg in self[k]:
                    vals.append(re.compile(fnmatch.translate(reg)))
                self[k]=vals

            elif ktype =='localpath':
