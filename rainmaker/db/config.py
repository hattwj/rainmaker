...
class Sync(Base):
    ...
    root = Column(String)
    tox_pri = Column(String)
    tox_bot_data = Column(String)

MIGRATIONS = {
    0 : """CREATE TABLE schema_migrations (version TEXT)""",
    1 : """ CREATE TABLE syncs (
    ...
    9 : """ CREATE VIEW differences AS
                SELECT DISTINCT 
                    m1.id AS my_file_id,
                    hsp.id AS host_sync_id
                FROM my_files m1
                LEFT JOIN host_syncs hsp
                    ON hsp.sync_id = m1.sync_id
